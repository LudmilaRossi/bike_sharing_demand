...
BIKEdata2 <- read_csv("BIKE2.csv")
BIKEdata2$registered=0
BIKEdata2$casual=0
BIKEdata2$count=0
BIKEdata=rbind(BIKEdata1,BIKEdata2)

#Exploracion
#primer paso en la exploracion. Comprobar que no hay datos vacios
table(is.na(BIKEdata))
#FALSE

hist(BIKEdata$season)
hist(BIKEdata$weather)

#ver la frecuencia relativa de valores de una columna
fs <- table(BIKEdata$season)
fw <- table(BIKEdata$weather)

#ver la frecuencia relativa de valores de una columna
prop.table(fw)
sub=data.frame(BIKEdata$registered,BIKEdata$casual,BIKEdata$count,BIKEdata$temp,BIKEdata$humidity,BIKEdata$atemp,BIKEdata$windspeed)
cor(sub)

#Transformamos la fecha de modo que pueda tener una tabla con los datos anteriores. En r, la coma final solo afecta a las filas, no a las columnas
train=BIKEdata[as.integer(substr(BIKEdata$datetime,9,10))<20,]
train

#sb=as.integer(substr(BIKEdata$datatime,12,13))
#sb

x <- rnorm(10) #crear una distribucion normal
x
